"""Compile raw company-name CSV into the five derived files the generator needs.

Input:  <Lang>.csv  — columns: name, industry
Output (same directory):
  <Lang>_cleared.csv              name, industry, frequency
  <Lang>_beginnings.csv           industry, beginning   (char n-grams 3-6)
  <Lang>_endings.csv              industry, ending      (char n-grams 3-9)
  <Lang>_separate_beginnings.csv  industry, separate_beginning, frequency
  <Lang>_separate_endings.csv     industry, separate_ending,    frequency

Usage (run from the onomaturgy-data repo root):
  python /path/to/onomaturgy/tools/compile_companies.py onomaturgy_data/csv/companies/namesets/RU/RU.csv [...]
"""
...
MIN_FREQ = 10  # minimum count (per industry) to qualify as a separate word
...
def compile_nameset(csv_path: str) -> None:
    ...
    all_cats = sorted({r["industry"] for r in raw})

    # ── Step 1: discover separate beginnings and endings ────────────────────
    first_freq: dict[tuple, int] = defaultdict(int)  # (word, industry) → count
    ...
    for r in raw:
        ...
        if len(parts) < 2:
            continue
        cat = r["industry"]
        first_freq[(parts[0],  cat)] += 1
        last_freq[ (parts[-1], cat)] += 1
    ...
    # ── Step 2: clear names ──────────────────────────────────────────────────
    cleared: dict[tuple, int] = defaultdict(int)  # (name, industry) → count
    ...
    write(f"{base}_separate_beginnings.csv", ["industry", "separate_beginning", "frequency"], beg_rows)
    ...
    write(f"{base}_separate_endings.csv", ["industry", "separate_ending", "frequency"], end_rows)
...
if __name__ == "__main__":
    ...
    for path in sys.argv[1:]:
        ...
        if stem != parent:
            continue  # skip derived files (e.g. RU_cleared.csv)
